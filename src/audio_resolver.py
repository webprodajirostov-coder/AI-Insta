...
def resolve_audio_asset(job_dir, asset_id):
    ...
    asset_path = job_dir / "media" / "audio" / f"{asset_id}.json"

    if not asset_path.exists():
        raise FileNotFoundError(f"AudioAsset not found: {asset_path}")
    ...
    if asset.get("status") != "ready":
        raise RuntimeError(
            f"Audio asset is not ready: status={asset.get('status')}"
        )

    if not asset.get("path"):
        raise RuntimeError("Audio asset has no physical path")
    ...
    try:
        ...
        raise ValueError(
            f"Audio asset path escapes job directory: {physical_path}"
        )

    if not physical_path.exists():
        raise FileNotFoundError(
            f"Audio file does not exist: {physical_path}"
        )
